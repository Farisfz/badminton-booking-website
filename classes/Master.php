...
Class Master extends DBConnection {
	...
	function save_booking(){
		...
		$check = $this->conn->query("SELECT * FROM `booking_list` where  facility_id = '{$facility_id}' and date_from = '{$date_from}' and ('{$date_to}' BETWEEN time(date_to) and time(time_end) or '{$time_end}' BETWEEN time(date_to) and time(time_end)) and status = 0")->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
